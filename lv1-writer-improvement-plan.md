...
**P0 — Genre/tone auto-detection + tone profiles (the explicit ask).**
Add a tone system with two independent knobs: a **genre/tone profile** (voice, structure, register) and a **rigor tier** (already exists: tiny/normal/full/high-stakes; controls how much proof apparatus ships). Triage detects the genre, selects a tone profile from a new profile library, records the choice and rationale, and the draft station composes the final voice. The inspector then checks the draft *against the chosen profile*. Reuse the existing review-mode classifier logic.
...
| Detected genre | Tone profile | Register | Proof apparatus |
|---|---|---|---|
| Intelligence / market / strategy report | `analytical-intelligence` | decisive, epistemic, hypothesis-driven (the axiom voice) | full: inline grades, methodology, steelman, appendix |
| Academic paper / literature review | `academic` | measured, formal, citation-dense | full, citation-style adapted |
| Narrative nonfiction / long-form journalism | `narrative-nonfiction` (current default) | vivid, case-led, warm-rigorous | medium: grades in appendix, lighter inline |
| Persuasive essay / op-ed | `persuasive` | committed, argument-forward, steelman required | medium |
| Technical doc / runbook / spec | `technical` | precise, neutral, imperative | low: spec-style refs, no narrative |
| Literary fiction | `literary` | stylistic, voice-first | none — fiction rules (consistency/craft) replace truth/citation |
| Romance | `romance` | emotional, intimate, sensory | none — fiction rules |
| Children's / YA | `childrens` | simple, age-tuned, safe | none — fiction rules |
...
**[new] `skills/lv1-writer/assets/tone-profiles/` (one file per profile)** — each is a self-contained voice spec in the same shape as the current `writing-instruction-template.md` (Spine & shape / Truth-or-fiction-contract / Detail / Tone & voice / Structure / Never-do). Ship at minimum: `analytical-intelligence.md`, `academic.md`, `narrative-nonfiction.md`, `persuasive.md`, `technical.md`, `literary.md`, `romance.md`, `childrens.md`. The current `writing-instruction-template.md` content becomes `narrative-nonfiction.md` almost verbatim. The `analytical-intelligence.md` profile should encode the axiom moves explicitly: hypothesis spine, per-section lens question, winners/losers synthesis, inline grades, mandatory steelman, methodology-first, honest-gap-in-text.
...
**[edit] `skills/lv1-writer-init/SKILL.md`** — at init, instead of always writing the science-narrative tone, either (a) leave `writing-instruction.md` unwritten and let per-task detection drive tone, or (b) ask the user for a project-wide default genre and seed `writing-instruction.md` from the matching profile. Recommend (a) as default with (b) offered, so a multi-genre project isn't locked to one voice. Report the available profiles.
...
1. **Tone system first** (3.1) — it's the explicit ask and everything else keys off the genre recorded in triage.
2. **Artifact-surfacing** (3.2) — biggest visible quality jump; mostly enforcement of existing rules.
...
The gap between the two reports is not writing skill — it is that axiom forces its rigor onto the delivered page (inline grades, methodology-and-enforcement section, hypothesis spine, mandatory steelman, conflict-debunking, a shipped proof package) while lv1-writer leaves that same rigor in its process files and ships a clean summary. lv1-writer is actually ahead on process machinery (claim ledger, coverage-gate, isolated agents). So the plan keeps that machinery and (1) adds the user's requested two-knob tone system — auto-detect genre, pick a tone profile, with fiction genres switching the proof apparatus *off* and craft contracts *on* — and (2) makes the axiom-grade apparatus ship inside the document, but only at the rigor level the detected genre warrants. The inspector is upgraded last to enforce all of it.
